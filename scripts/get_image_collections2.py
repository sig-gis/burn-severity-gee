...
def doIndices(collection):
    ...
    return ee.ImageCollection(collection).map(indicesMapper)
...
def mergeLandsatCols(baseCollection, newCollection, start, end, region, func):
    ...
    filteredNewCollectionSize = filteredNewCollection.toList(1).size()#.getInfo() # can't getInfo() inside a mapped function
    ...
    # def do_merge():
    #     fnc = filteredNewCollection.map(func,True)
    #     bc = baseCollection.merge(filteredNewCollection)
    #     return baseC
    baseCollection = ee.Algorithms.If(filteredNewCollectionSize, 
                                      ee.ImageCollection(baseCollection).merge(filteredNewCollection.map(func,True)),
                                      baseCollection)
